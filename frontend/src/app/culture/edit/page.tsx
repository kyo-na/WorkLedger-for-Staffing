                  ...
                  <div key={key} className="group">
                    <div className="flex justify-between items-end mb-3">
                      <span className="font-bold text-slate-600 text-xs flex items-center gap-2">
                        <span className={`w-2 h-2 rounded-full ${formData.traits[key] > 70 ? 'bg-indigo-500' : formData.traits[key] < 30 ? 'bg-slate-300' : 'bg-indigo-300'}`}></span>
                        {key === 'agreeableness' ? '協調性 (Agreeableness)' : 
                         key === 'extraversion' ? '外向性 (Extraversion)' : 
                         key === 'conscientiousness' ? '誠実性 (Conscientiousness)' : 
                         key === 'openness' ? '開放性 (Openness)' : '情緒安定性 (Emotional Stability)'}
                      </span>
                      <span className="text-2xl font-black text-slate-800 tabular-nums">
                        {formData.traits[key]}<span className="text-[10px] ml-0.5 font-bold text-slate-300">%</span>
                      </span>
                    </div>
                    ...
                    <div className="relative h-6 flex items-center">
                      ...
                      <div className="absolute w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                         <div className="h-full bg-gradient-to-r from-indigo-200 to-indigo-500 transition-all duration-300" style={{width: `${formData.traits[key]}%`}}></div>
                      </div>
                      ...
                      {/* Custom Thumb */}
                      <div 
                        className="absolute h-5 w-5 bg-white border-2 border-indigo-500 rounded-full shadow-md z-10 pointer-events-none transition-all duration-100 group-hover:scale-125"
                        style={{ left: `calc(${formData.traits[key]}% - 10px)` }}
                      ></div>
                    </div>
                  </div>
                ))}
